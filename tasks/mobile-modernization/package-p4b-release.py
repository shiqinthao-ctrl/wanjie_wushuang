"""Preserve P4b browser evidence and package the clean tagged preview."""
import hashlib
import json
import subprocess
import sys
...
EVIDENCE = ROOT / "tasks/mobile-modernization/evidence/P4b"
TAG = "mobile-next-p4b-20260909"
...
def media():
    ...
    before = next((EVIDENCE / "before-02/test-results").glob("*clear-short/classic.png"))
    after = next((EVIDENCE / "after-01/test-results").glob("*clear-short/classic.png"))
    poster = Image.new("RGB", (640, 640), "#122522")
    draw = ImageDraw.Draw(poster)
    font = ImageFont.truetype("C:/Windows/Fonts/msyh.ttc", 20)
    draw.text((12, 8), "P4b | \u77ed\u5c4f\u624b\u673a\u6218\u6597\u5e03\u5c40 320 x 568", font=font, fill="#eadcaa")
    draw.text((12, 40), "\u4fee\u590d\u524d\uff1a\u6587\u5b57\u91cd\u53e0", font=font, fill="white")
    draw.text((332, 40), "\u4fee\u590d\u540e\uff1a\u5206\u533a\u6e05\u6670", font=font, fill="white")
    poster.paste(Image.open(before).convert("RGB"), (0, 72))
    poster.paste(Image.open(after).convert("RGB"), (320, 72))
    poster.save(OUTPUT / "comparison.png")
    source = next((EVIDENCE / "after-03/test-results").glob("*height-changes-short/video.webm"))
    video = OUTPUT / "wanjie-p4b-natural-test.mp4"
    run("ffmpeg", "-v", "error", "-n", "-i", str(source), "-c:v", "libx264", "-crf", "20",
        "-pix_fmt", "yuv420p", "-movflags", "+faststart", str(video))
    frames = Image.new("RGB", (1280, 568), "#122522")
    for index, seconds in enumerate([1, 3, 7, 10]):
        frame = OUTPUT / f"frame-{seconds}.png"
        run("ffmpeg", "-v", "error", "-n", "-ss", str(seconds), "-i", str(video), "-frames:v", "1", str(frame))
        frames.paste(Image.open(frame).convert("RGB"), (320 * index, 0))
...
def release():
    ...
    for attempt, count in [("after-02", 3), ("after-03", 6), ("regression-01", 18), ("scroll-01", 3)]:
        stats = json.loads((EVIDENCE / attempt / "results.json").read_text(encoding="utf8"))["stats"]
        assert stats["expected"] == count
        assert stats["unexpected"] == (6 if attempt == "after-02" else 0)
    recordings = json.loads((EVIDENCE / "recordings.json").read_text(encoding="utf8"))["recordings"]
    assert len(recordings) == 42
    for item in recordings:
        assert item["fullDecodePassed"] and digest(EVIDENCE / item["path"]) == item["sha256"]
    public = ROOT / "dist/render"
    assets = [package("wanjie-p4b-render-package.zip", [(p, p.relative_to(public).as_posix())
        for p in sorted(public.rglob("*")) if p.is_file()], commit),
        package("wanjie-p4b-test-recordings.zip", [(p, p.relative_to(EVIDENCE).as_posix())
        for p in sorted(EVIDENCE.rglob("*")) if p.is_file()], commit)]
    notes = OUTPUT / "RELEASE-NOTES.md"
    notes.write_bytes((ROOT / "tasks/mobile-modernization/P4b-EVIDENCE.md").read_bytes())
    assets.extend([OUTPUT / "wanjie-p4b-natural-test.mp4", OUTPUT / "comparison.png", notes])
