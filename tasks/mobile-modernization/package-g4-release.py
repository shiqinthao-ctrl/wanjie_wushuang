"""Preserve G4 browser evidence and package the clean tagged preview."""
import hashlib
import json
import subprocess
import sys
...
EVIDENCE = ROOT / "tasks/mobile-modernization/evidence/G4"
TAG = "mobile-next-g4-20260909"
...
def media():
    ...
    for profile in ["desktop", "phone"]:
        ...
        video = OUTPUT / f"wanjie-g4-{profile}-full.mp4"
    ...
    for index, seconds in enumerate([29, 80, 145, 295]):
        frame = OUTPUT / f"phone-frame-{seconds}.png"
        run("ffmpeg", "-v", "error", "-n", "-ss", str(seconds), "-i", str(OUTPUT / "wanjie-g4-phone-full.mp4"), "-frames:v", "1", str(frame))
        shot = Image.open(frame).convert("RGB")
        shot.thumbnail((312, 800))
        x = index * 320 + (320 - shot.width) // 2
        sheet.paste(shot, (x, 44))
        draw.text((index * 320 + 12, 8), f"G4 | {seconds}s", font=font, fill="#eadcaa")
    sheet.save(OUTPUT / "recording-review.png")

    poster = Image.new("RGB", (960, 900), "#122522")
    draw = ImageDraw.Draw(poster)
    draw.text((18, 10), "G4 | \u5c9a\u5f71\u5251\u5c0a / \u4f0f\u9635\u98ce\u66b4 / \u98ce\u5f71\u5408\u88ad", font=font, fill="#eadcaa")
    for index, filename in enumerate(["form-windwarden.png", "route-ambush.png", "evolution-bond.png"]):
    ...
    poster.save(OUTPUT / "g4-build-preview.png")
...
def release():
    ...
    assert rules["numPassedTests"] == 960 and rules["numFailedTests"] == 0
    ...
    assets = [package("wanjie-g4-render-package.zip", [(p, p.relative_to(public).as_posix())
        for p in sorted(public.rglob("*")) if p.is_file()], commit),
        package("wanjie-g4-test-recordings.zip", [(p, p.relative_to(EVIDENCE).as_posix())
        for p in sorted(EVIDENCE.rglob("*")) if p.is_file()], commit)]
    notes = OUTPUT / "RELEASE-NOTES.md"
    notes.write_bytes((ROOT / "tasks/mobile-modernization/G4-EVIDENCE.md").read_bytes())
    assets.extend([OUTPUT / item["file"] for item in media_info["demos"]])
    assets.extend([OUTPUT / "g4-build-preview.png", notes])
