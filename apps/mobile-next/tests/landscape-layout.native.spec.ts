...
async function checkLayout(page: Page, info: TestInfo, label: string, sides = 0, bottom = 0) {
  ...
  for (const box of boxes) {
    expect.soft(box.x, box.selector).toBeGreaterThanOrEqual(sides);
    expect.soft(box.y, box.selector).toBeGreaterThanOrEqual(0);
    expect.soft(box.x + box.width, box.selector).toBeLessThanOrEqual(viewport.width - sides);
    expect.soft(box.y + box.height, box.selector).toBeLessThanOrEqual(viewport.height - bottom);
  }
  ...
  for (const box of boxes) {
    ...
    expect.soft(hero.x + 33 > box.x && hero.x - 33 < box.x + box.width &&
      hero.y + 13 > box.y && hero.y - 68 < box.y + box.height, `${label}: hero under ${box.selector}`).toBe(false);
  }
...
test('landscape HUD keeps hero, rewards and controls clear', async ({ page }, info) => {
  test.skip(!info.project.name.startsWith('landscape-'), 'Landscape slice');
  const errors: string[] = [];
...
test('landscape evolution supports touch, upgrades, safe areas and rotation', async ({ page }, info) => {
  test.skip(!info.project.name.startsWith('landscape-'), 'Landscape slice');
  ...
  try {
    await touch.send('Emulation.setSafeAreaInsetsOverride', { insets: { top: 0, bottom: 21, left: 44, right: 44 } });
    await expect(page.locator('.battle-overview')).toHaveCSS('padding-left', '44px');
    await checkLayout(page, info, 'evolution-safe-area', 44, 21);
    ...
    for (const viewport of [{ width: 320, height: 568 }, { width: 390, height: 844 },
      { width: 568, height: 300 }, { width: 844, height: 390 }, info.project.use.viewport!]) {
    ...
    await checkLayout(page, info, 'resumed', 44, 21);
    await page.setViewportSize({ width: info.project.use.viewport!.width, height: 300 });
    await expect(async () => {
      expect((await page.locator('canvas').boundingBox())!.height).toBe(300);
    }).toPass();
    await checkLayout(page, info, 'browser-bars', 44, 21);
    await page.getByRole('button', { name: '暂停', exact: true }).click();
...
test('synthetic crowded landscape HUD keeps boss, notices and rewards clear', async ({ page }, info) => {
  test.skip(!info.project.name.startsWith('landscape-'), 'Landscape slice');
  await page.goto('./');
  await page.getByRole('button', { name: '进化征途', exact: true }).click();
  await page.getByRole('button', { name: '开启进化征途' }).click();
  ...
  try {
    ...
    await checkLayout(page, info, 'synthetic-crowded', 44, 21);
